...
- Logo and color are now supplied. A [vector reconstruction and transparent PNG set](../public/brand/README.md) is available; the horizontal SVG is used in the storefront header and footer. The reconstruction awaits client visual review.

## Domain and business accounts

- Client believes a previous domain may have expired and asks for it to be checked. The supplied name is `iyayusufspantry`, with no domain extension. She says it would have been registered through Shopify if she had one. Exact domain, ownership, registrar, and expiry remain unverified.
- Client is comfortable with the developer creating a dedicated Google account and requests the name `iyayusufspantry`. Availability and account creation are pending. The client should retain ownership and recovery access, as stated in the preceding project email.
...
- Vercel and Contentful were mentioned as services that could be managed using the business account. This correspondence does not confirm provisioning, subscriptions, or final platform choices.
...
- Current product range, descriptions, variants, prices, stock, and approved product photography remain to be confirmed. Existing sample catalogue entries should not be treated as approved inventory.
...
- Initial **US$250 received and acknowledged on 19 September 2026**. Remaining **US$250** is due after final review and approval, before launch.
